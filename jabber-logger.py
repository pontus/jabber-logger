...
p.read( homed + ".jabberloggerrc" )
PN = 'jabberlogger'

if not p.has_option(PN,"jid") or not p.has_option(PN,"password"):
    print u"Please configure jid and password in %s" % (
        homed+".jabberloggerrc")
    sys.exit(1)

print u"creating client..."
c=Client(JID(p.get(PN,"jid")) ,p.get(PN,"password"))
